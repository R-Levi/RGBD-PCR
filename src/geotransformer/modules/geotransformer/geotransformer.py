...
from src.geotransformer.modules.ops import pairwise_distance
from src.geotransformer.modules.transformer import SinusoidalPositionalEmbedding, RPEConditionalTransformer
@torch.no_grad()
...
class GeometricStructureEmbedding(nn.Module):
    ...
    @torch.no_grad()
    def get_embedding_indices(self, points,overlapped_points_c_idx,no_overlapped_points_c_idx):
        r"""Compute the indices of pair-wise distance embedding and triplet-wise angular embedding.
        ...
        try:
            if no_overlapped_points_c_idx.shape[0]>0:
                a_indices[:,:,no_overlapped_points_c_idx,:] = 0
                d_indices[:,:,no_overlapped_points_c_idx]= 0
        except:
            print(no_overlapped_points_c_idx.shape[0])

        # try:
        #     if overlapped_points_c_idx.shape[0]>0:
        #         a_indices[:,:,overlapped_points_c_idx,:] = 0
        #         d_indices[:,:,overlapped_points_c_idx]= 0
        # except:
        #     print(no_overlapped_points_c_idx.shape[0])
    ...
    def forward(self, points,overlapped_points_c_idx,no_overlapped_points_c_idx):
        d_indices, a_indices = self.get_embedding_indices(points,overlapped_points_c_idx,no_overlapped_points_c_idx)
...
class GeometricTransformer(nn.Module):
    ...
    def forward(
        ...
        """
        ref_embeddings,init_ref_embedding = self.embedding(ref_points,ref_overlapped_points_c_idx,ref_no_overlapped_points_c_idx)
        src_embeddings ,init_src_embedding = self.embedding(src_points,src_overlapped_points_c_idx,src_no_overlapped_points_c_idx)
